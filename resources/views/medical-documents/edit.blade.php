{{-- resources/views/medical-documents/edit.blade.php --}}
@extends('layouts.app')
...
<div class="max-w-4xl mx-auto">
    ...
    <div class="mb-6 sm:mb-8">
        <div class="flex items-center mb-4">
            <a href="{{ route('medical-documents.show', $medicalDocument) }}"
               class="text-gray-600 hover:text-gray-900 transition-colors mr-4">
                <i class="fas fa-arrow-left text-lg"></i>
            </a>
            <div>
                <h1 class="text-2xl sm:text-3xl font-bold text-gray-900 mb-2">Edytuj dokument medyczny</h1>
                <p class="text-gray-600 text-sm sm:text-base">Modyfikuj dokument dla pacjenta {{ $medicalDocument->patient->full_name }}</p>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white rounded-xl shadow-lg overflow-hidden">
        <form method="POST" action="{{ route('medical-documents.update', $medicalDocument) }}" enctype="multipart/form-data" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="p-6 space-y-6">
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                        ...
                        <select name="patient_id"
                            ...
                            @foreach($patients as $patient)
                                <option value="{{ $patient->id }}"
                                        {{ (old('patient_id', $medicalDocument->patient_id) == $patient->id) ? 'selected' : '' }}>
                    ...
                    <div>
                        ...
                        <input type="text"
                               name="title"
                               id="title"
                               value="{{ old('title', $medicalDocument->title) }}"
                    ...
                    <div>
                        ...
                        <select name="type"
                            ...
                            <option value="general" {{ old('type', $medicalDocument->type) == 'general' ? 'selected' : '' }}>Ogólny</option>
                            <option value="diagnosis" {{ old('type', $medicalDocument->type) == 'diagnosis' ? 'selected' : '' }}>Diagnoza</option>
                            <option value="treatment" {{ old('type', $medicalDocument->type) == 'treatment' ? 'selected' : '' }}>Leczenie</option>
                            <option value="examination" {{ old('type', $medicalDocument->type) == 'examination' ? 'selected' : '' }}>Badanie</option>
                            <option value="prescription" {{ old('type', $medicalDocument->type) == 'prescription' ? 'selected' : '' }}>Recepta</option>
                        </select>
                        @error('type')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        <input type="date"
                               name="document_date"
                               id="document_date"
                               value="{{ old('document_date', $medicalDocument->document_date->format('Y-m-d')) }}"
                    ...
                    <div>
                        ...
                        <select name="status"
                                ...
                                required>
                            <option value="draft" {{ old('status', $medicalDocument->status) == 'draft' ? 'selected' : '' }}>Szkic</option>
                            <option value="completed" {{ old('status', $medicalDocument->status) == 'completed' ? 'selected' : '' }}>Ukończony</option>
                            <option value="archived" {{ old('status', $medicalDocument->status) == 'archived' ? 'selected' : '' }}>Zarchiwizowany</option>
                        </select>
                        @error('status')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                ...
                <div>
                    ...
                    <textarea name="content"
                              ...
                              required>{{ old('content', $medicalDocument->content) }}</textarea>
                    @error('content')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <textarea name="notes"
                              ...
                              class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 @error('notes') @enderror">{{ old('notes', $medicalDocument->notes) }}</textarea>
                    @error('notes')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                    ...
                    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                        ...
                        <div>
                            ...
                            <textarea name="symptoms"
                                      ...
                                      class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 @error('symptoms') @enderror">{{ old('symptoms', isset($medicalDocument->metadata['symptoms']) ? implode("\n", $medicalDocument->metadata['symptoms']) : '') }}</textarea>
                            @error('symptoms')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div>
                            ...
                            <textarea name="medications"
                                      ...
                                      class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 @error('medications') @enderror">{{ old('medications', isset($medicalDocument->metadata['medications']) ? implode("\n", $medicalDocument->metadata['medications']) : '') }}</textarea>
                            @error('medications')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div>
                            ...
                            <textarea name="recommendations"
                                      ...
                                      class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 @error('recommendations') @enderror">{{ old('recommendations', isset($medicalDocument->metadata['recommendations']) ? implode("\n", $medicalDocument->metadata['recommendations']) : '') }}</textarea>
                ...
                <div class="bg-blue-50 border border-blue-200 rounded-lg p-4">
                    <h4 class="text-sm font-medium text-blue-900 mb-2">Obecny załącznik</h4>
                    <div class="flex items-center justify-between">
                        <div class="flex items-center">
                            <i class="fas fa-file-alt text-blue-600 mr-3"></i>
                            <div>
                                <div class="text-sm font-medium text-blue-900">{{ $medicalDocument->file_name }}</div>
                                <div class="text-xs text-blue-700">Kliknij "Zastąp plik" aby zmienić załącznik</div>
                            </div>
                        </div>
                        <div class="flex items-center space-x-2">
                            ...
                            <button type="button"
                                    onclick="deleteCurrentFile({{ $medicalDocument->id }})"
                ...
                <div>
                    <label for="document_file" class="block text-sm font-medium text-gray-700 mb-2">
                        {{ $medicalDocument->hasFile() ? 'Zastąp załącznik' : 'Dodaj załącznik' }}
                    </label>
                    ...
                    <div class="flex items-start">
                        <div class="flex items-center h-5">
                            <input id="is_private"
                                   ...
                                   type="checkbox"
                                   value="1"
                                   {{ old('is_private', $medicalDocument->is_private) ? 'checked' : '' }}
...
function deleteCurrentFile(documentId) {
    if (confirm('Czy na pewno chcesz usunąć obecny plik?')) {
