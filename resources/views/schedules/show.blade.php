    ...
    <div class="max-w-4xl mx-auto">
        <div class="mb-6 flex items-center justify-between">
            <div>
                <h1 class="text-3xl font-bold text-gray-900 dark:text-white mb-2">
                    Harmonogram - {{ $doctor->full_name }}
                </h1>
                <p class="text-gray-600 dark:text-gray-400">Edytuj harmonogram pracy dla tego fizjoterapeuty</p>
            </div>
            <a href="{{ route('schedules.index') }}" class="flex items-center text-indigo-600 hover:text-indigo-700 dark:text-indigo-400">
        ...
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow-md p-6 mb-6">
            ...
            <div class="flex flex-wrap gap-3">
                <form action="{{ route('schedules.default', $doctor->id) }}" method="POST" class="inline">
                    @csrf
                    <button type="submit" class="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg transition">
                        Ustaw domyślny harmonogram (Pon-Pt 8:00-16:00)
                    </button>
                </form>
                <form action="{{ route('schedules.clear', $doctor->id) }}" method="POST" class="inline" onsubmit="return confirm('Czy na pewno chcesz wyczyścić cały harmonogram?');">
                    @csrf
                    @method('DELETE')
        ...
        <form action="{{ route('schedules.update', $doctor->id) }}" method="POST">
            @csrf
            ...
            <div class="mt-6 flex justify-end gap-3">
                <a href="{{ route('schedules.index') }}" class="bg-gray-300 hover:bg-gray-400 dark:bg-gray-600 dark:hover:bg-gray-500 text-gray-800 dark:text-white px-6 py-3 rounded-lg transition">
                    Anuluj
                </a>
